...
let bgmOscillators = []; // BGM 오실레이터들을 저장
let bgmGainNode = null;
let isBGMPlaying = false;
let currentBGMType = 'normal'; // 'normal', 'boss100', 'boss200'

// 일반 BGM 재생 함수 (레트로 스타일 루프)
const playBGM = () => {
  ...
  bgmGainNode.gain.value = 0.15; // BGM 볼륨 (조용하게)
  bgmGainNode.connect(audioContext.destination);

  // 멜로디 패턴 (레트로 슈팅 게임 스타일)
  const melody = [
    ...
    { note: 587.33, duration: 0.2 }, // D5
    { note: 659.25, duration: 0.2 }, // E5
    { note: 523.25, duration: 0.4 }, // C5 (longer)
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'normal') {
      return;
    }
    ...
    if (note > 0) {
      ...
      oscillator.type = 'square'; // 레트로 사운드
      oscillator.frequency.setValueAtTime(note, currentTime);
      oscillator.connect(bgmGainNode);
...
// 100 레벨 보스 BGM (강렬하고 긴박한 느낌)
const playBoss100BGM = () => {
  ...
  isBGMPlaying = true;
  currentBGMType = 'boss100';
  bgmGainNode = audioContext.createGain();
  bgmGainNode.gain.value = 0.2; // 보스 BGM은 조금 더 크게
  bgmGainNode.connect(audioContext.destination);

  // 강렬한 보스 멜로디 (마이너 스케일, 빠른 템포)
  const melody = [
    ...
    { note: 466.16, duration: 0.3 },  // A#4 (longer)
    { note: 440.00, duration: 0.15 }, // A4
    { note: 392.00, duration: 0.15 }, // G4
    { note: 349.23, duration: 0.15 }, // F4
    { note: 392.00, duration: 0.15 }, // G4
    { note: 440.00, duration: 0.3 },  // A4 (longer)
    { note: 0, duration: 0.05 }       // short pause
  ];
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss100') {
      return;
    }
    ...
    if (note > 0) {
      // 메인 멜로디
      const oscillator = audioContext.createOscillator();
      oscillator.type = 'sawtooth'; // 더 강렬한 사운드
      oscillator.frequency.setValueAtTime(note, currentTime);
      oscillator.connect(bgmGainNode);
      oscillator.start(currentTime);
      oscillator.stop(currentTime + duration);
      ...
      bass.type = 'square';
      bass.frequency.setValueAtTime(note / 2, currentTime);
      bassGain.gain.value = 0.3;
      bass.connect(bassGain);
      bassGain.connect(audioContext.destination);
      bass.start(currentTime);
      bass.stop(currentTime + duration);
...
// 200 레벨 최종 보스 BGM (웅장하고 압도적인 느낌)
const playBoss200BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM(); // 기존 BGM 정지
  ...
  isBGMPlaying = true;
  currentBGMType = 'boss200';
  bgmGainNode = audioContext.createGain();
  bgmGainNode.gain.value = 0.25; // 최종 보스는 가장 크게
  bgmGainNode.connect(audioContext.destination);

  // 웅장한 최종 보스 멜로디 (에픽한 느낌)
  const melody = [
    { note: 329.63, duration: 0.2 },  // E4
    { note: 329.63, duration: 0.2 },  // E4
    { note: 329.63, duration: 0.2 },  // E4
    { note: 493.88, duration: 0.4 },  // B4 (longer)
    { note: 440.00, duration: 0.2 },  // A4
    { note: 392.00, duration: 0.2 },  // G4
    { note: 329.63, duration: 0.2 },  // E4
    { note: 261.63, duration: 0.4 },  // C4 (longer)
    { note: 329.63, duration: 0.2 },  // E4
    { note: 392.00, duration: 0.2 },  // G4
    { note: 440.00, duration: 0.2 },  // A4
    { note: 493.88, duration: 0.6 },  // B4 (longest)
    { note: 0, duration: 0.1 }        // pause
  ];
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss200') {
      return;
    }
    ...
    if (note > 0) {
      ...
      harmony.type = 'square';
      harmony.frequency.setValueAtTime(note * 1.5, currentTime);
      harmonyGain.gain.value = 0.2;
      harmony.connect(harmonyGain);
      harmonyGain.connect(audioContext.destination);
      harmony.start(currentTime);
      harmony.stop(currentTime + duration);
      bgmOscillators.push(harmony);
      
      // 강력한 베이스 (두 옥타브 아래)
      const bass = audioContext.createOscillator();
      const bassGain = audioContext.createGain();
      bass.type = 'square';
      bass.frequency.setValueAtTime(note / 4, currentTime);
...
// 각 보스별 고유 BGM 함수들
// 보스 1: Iron Drone (10레벨) - 기계적이고 리드미컬한 느낌
const playBoss1BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM();
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss1') return;
    const { note, duration } = melody[noteIndex];
    if (note > 0) {
...
// 보스 2: Plasma Reaper (20레벨) - 빠르고 공격적인 느낌
const playBoss2BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM();
  if (!audioContext) audioContext = createAudioContext();
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss2') return;
    const { note, duration } = melody[noteIndex];
    if (note > 0) {
      const osc = audioContext.createOscillator();
      osc.type = 'sawtooth';
...
// 보스 3: Shadow Warden (30레벨) - 어둡고 신비로운 느낌
const playBoss3BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM();
  if (!audioContext) audioContext = createAudioContext();
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss3') return;
    const { note, duration } = melody[noteIndex];
    if (note > 0) {
      const osc = audioContext.createOscillator();
      osc.type = 'triangle';
...
// 보스 4: Omega Core (40레벨) - 안정적이면서도 위협적인 느낌
const playBoss4BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM();
  if (!audioContext) audioContext = createAudioContext();
  ...
  isBGMPlaying = true;
  currentBGMType = 'boss4';
  bgmGainNode = audioContext.createGain();
  bgmGainNode.gain.value = 0.19;
  bgmGainNode.connect(audioContext.destination);
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss4') return;
    const { note, duration } = melody[noteIndex];
    if (note > 0) {
      const osc = audioContext.createOscillator();
      const bass = audioContext.createOscillator();
...
// 보스 5: Void Overlord (50레벨) - 공포스럽고 압도적인 느낌
const playBoss5BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM();
  if (!audioContext) audioContext = createAudioContext();
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss5') return;
    const { note, duration } = melody[noteIndex];
    if (note > 0) {
      const osc = audioContext.createOscillator();
      osc.type = 'sawtooth';
...
// 보스 6: Storm Striker (60레벨) - 전기적이고 격렬한 느낌
const playBoss6BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM();
  if (!audioContext) audioContext = createAudioContext();
  ...
  isBGMPlaying = true;
  currentBGMType = 'boss6';
  bgmGainNode = audioContext.createGain();
  bgmGainNode.gain.value = 0.19;
  bgmGainNode.connect(audioContext.destination);

  const melody = [
    { note: 587.33, duration: 0.12 }, // D5
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss6') return;
    const { note, duration } = melody[noteIndex];
    if (note > 0) {
      const osc = audioContext.createOscillator();
      osc.type = 'square';
...
// 보스 7: Time Twister (70레벨) - 불규칙하고 시간을 왜곡하는 느낌
const playBoss7BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM();
  if (!audioContext) audioContext = createAudioContext();
  ...
  isBGMPlaying = true;
  currentBGMType = 'boss7';
  bgmGainNode = audioContext.createGain();
  bgmGainNode.gain.value = 0.19;
  bgmGainNode.connect(audioContext.destination);

  const melody = [
    { note: 440.00, duration: 0.18 }, // A4
    { note: 523.25, duration: 0.15 }, // C5
    { note: 587.33, duration: 0.12 }, // D5
    { note: 698.46, duration: 0.18 }, // F5
    { note: 587.33, duration: 0.15 }, // D5
    { note: 523.25, duration: 0.22 }, // C5
  ];
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss7') return;
    const { note, duration } = melody[noteIndex];
    if (note > 0) {
      const osc = audioContext.createOscillator();
      osc.type = 'triangle';
...
// 보스 8: Star Destroyer (80레벨) - 우주적이고 웅장한 느낌
const playBoss8BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM();
  if (!audioContext) audioContext = createAudioContext();
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss8') return;
    const { note, duration } = melody[noteIndex];
    if (note > 0) {
      const osc = audioContext.createOscillator();
      ...
      harm.type = 'sine';
      harm.frequency.setValueAtTime(note * 2, currentTime);
      harmGain.gain.value = 0.2;
      harm.connect(harmGain);
      harmGain.connect(audioContext.destination);
      harm.start(currentTime);
      harm.stop(currentTime + duration);
      
      bgmOscillators.push(osc, harm);
    }
    noteIndex = (noteIndex + 1) % melody.length;
    currentTime += duration;
    setTimeout(playNote, duration * 1000);
...
// 보스 9: Chaos Emperor (90레벨) - 혼돈스럽고 파괴적인 느낌
const playBoss9BGM = () => {
  if (!isSoundEnabled) return;
  stopBGM();
  if (!audioContext) audioContext = createAudioContext();
  ...
  const playNote = () => {
    if (!isSoundEnabled || !isBGMPlaying || currentBGMType !== 'boss9') return;
    const { note, duration } = melody[noteIndex];
    if (note > 0) {
      const osc = audioContext.createOscillator();
      osc.type = 'sawtooth';
...
function PixelClassicShooter() {
  ...
  useEffect(() => {
    ...
    // 사운드가 꺼지면 BGM 정지, 켜지면 게임 중일 때만 BGM 재생
    if (!soundEnabled) {
      stopBGM();
  ...
  // ----- spawn wave (reads barrier refs) -----
    ...
    if (isBossWave) {
      // 보스별 고유 BGM 전환
      const bossLevel = levelNum % 200; // 200 넘어가면 순환
      if (levelNum === 200) {
        playBoss200BGM(); // 200레벨 최종 보스 BGM
      } else if (levelNum === 100) {
        playBoss100BGM(); // 100레벨 보스 BGM
      } else if (bossLevel === 10) {
        playBoss1BGM(); // Iron Drone
      } else if (bossLevel === 20) {
        playBoss2BGM(); // Plasma Reaper
      } else if (bossLevel === 30) {
        playBoss3BGM(); // Shadow Warden
      } else if (bossLevel === 40) {
        playBoss4BGM(); // Omega Core
      } else if (bossLevel === 50) {
        playBoss5BGM(); // Void Overlord
      } else if (bossLevel === 60) {
        playBoss6BGM(); // Storm Striker
      } else if (bossLevel === 70) {
        playBoss7BGM(); // Time Twister
      } else if (bossLevel === 80) {
        playBoss8BGM(); // Star Destroyer
      } else if (bossLevel === 90) {
        playBoss9BGM(); // Chaos Emperor
      }
  ...
  function applyUpgrade(type) {
    ...
    if (type === "playerStarfall") {
      ...
      if (!playerStatsRef.current.hasStarfall) {
        ...
        playerStatsRef.current.starfallLevel = (playerStatsRef.current.starfallLevel || 1) + 2;
        setPlayerStats((p) => {
          const nv = { ...p, attackPower: (p.attackPower || 1) + 2 };
          playerStatsRef.current = nv;
          return nv;
    ...
    if (type === "playerChaos") {
      // 카오스 공격 (중첩 시 확률 증가 + 강력한 데미지)
      if (!playerStatsRef.current.hasChaos) {
        playerStatsRef.current.hasChaos = true;
        playerStatsRef.current.chaosLevel = 2;
        setPlayerStats((p) => {
          const nv = { ...p, attackPower: (p.attackPower || 1) + 4, shield: p.shield + 2 };
          playerStatsRef.current = nv;
          return nv;
        });
      } else {
        playerStatsRef.current.chaosLevel = (playerStatsRef.current.chaosLevel || 1) + 2;
        setPlayerStats((p) => {
          const nv = { ...p, attackPower: (p.attackPower || 1) + 3 };
          playerStatsRef.current = nv;
          return nv;
  ...
  useEffect(() => {
      ...
      for (const e of st.enemies) {
        if (e.boss) {
          ...
          if (e.skill === "tripleShot") {
            ...
            ctx.fillStyle = "#ff0000";
            ctx.fillRect(e.x + 5, e.y + 3, 6, 6); // 빨간 눈
            ctx.fillRect(e.x + e.w - 11, e.y + 3, 6, 6); // 빨간 눈
            ctx.fillStyle = "#666666";
            ctx.fillRect(e.x + 3, e.y + e.h - 4, 4, 4); // 엔진
            ctx.fillRect(e.x + e.w - 7, e.y + e.h - 4, 4, 4); // 엔진
          } else if (e.skill === "rapidFire") {
            // Plasma Reaper - 플라즈마 형태
            ctx.fillStyle = "#00ffff";
            ctx.fillRect(e.x + 5, e.y, e.w - 10, e.h); // 중앙 몸체
            ctx.fillStyle = "#0088ff";
            ctx.fillRect(e.x, e.y + 5, e.w, e.h - 10); // 가로 날개
            ctx.fillStyle = "#ffffff";
            ctx.fillRect(e.x + e.w/2 - 2, e.y + 3, 4, 4); // 플라즈마 코어
          } else if (e.skill === "teleport") {
            // Shadow Warden - 삼각형 스텔스
            ...
            ctx.fillRect(e.x + e.w/2 - 3, e.y + 5, 6, 6); // 중앙 보라색 코어
            ...
            ctx.fillRect(centerX - 3, centerY - 3, 6, 6); // 중앙 코어
          } else if (e.skill === "final") {
            // Void Overlord - 십자가 형태
            ctx.fillStyle = "#ff00ff";
            ctx.fillRect(e.x + e.w/2 - 4, e.y, 8, e.h); // 세로 막대
            ctx.fillRect(e.x, e.y + e.h/2 - 4, e.w, 8); // 가로 막대
            ctx.fillStyle = "#ff00aa";
            ctx.fillRect(e.x + e.w/2 - 5, e.y + e.h/2 - 5, 10, 10); // 중앙 코어
          } else if (e.skill === "lightning") {
            // Storm Striker - 번개 모양
            ctx.fillStyle = "#ffff00";
            ctx.fillRect(e.x + 5, e.y, e.w - 10, e.h); // 본체
            ctx.fillStyle = "#ffffff";
            // 번개 장식
            ctx.fillRect(e.x + 3, e.y + 4, 3, 3);
            ctx.fillRect(e.x + e.w - 6, e.y + 4, 3, 3);
            ctx.fillRect(e.x + e.w/2 - 2, e.y + 2, 4, 4); // 상단 전기
            ctx.fillStyle = "#ffcc00";
            ctx.fillRect(e.x, e.y + 6, e.w, 6); // 날개
            ...
            const centerY = e.y + e.h/2;
            ctx.beginPath();
            ctx.arc(centerX, centerY, e.w/2, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = "#ffffff";
            ctx.fillRect(centerX - 1, centerY - 6, 2, 6); // 시침
            ctx.fillRect(centerX - 1, centerY - 1, 5, 2); // 분침
            ...
            for (let i = 0; i < 5; i++) {
              ...
              const outerX = centerX + Math.cos(angle) * 12;
              const outerY = centerY + Math.sin(angle) * 12;
              const innerAngle = angle + Math.PI / 5;
              const innerX = centerX + Math.cos(innerAngle) * 6;
              const innerY = centerY + Math.sin(innerAngle) * 6;
            ...
            // Chaos Emperor - 카오스 형태
            ctx.fillStyle = "#ff0000";
            ctx.fillRect(e.x + 3, e.y + 3, e.w - 6, e.h - 6); // 내부
            ctx.fillStyle = "#000000";
            ctx.fillRect(e.x, e.y, e.w, 3); // 상단
            ctx.fillRect(e.x, e.y + e.h - 3, e.w, 3); // 하단
            ctx.fillRect(e.x, e.y, 3, e.h); // 좌측
            ctx.fillRect(e.x + e.w - 3, e.y, 3, e.h); // 우측
            ctx.fillStyle = "#ff00ff";
            // 카오스 패턴
            for (let i = 0; i < 4; i++) {
              ctx.fillRect(e.x + 5 + i * 5, e.y + 5, 3, 3);
              ctx.fillRect(e.x + 5 + i * 5, e.y + e.h - 8, 3, 3);
            }
          } else if (e.skill === "ultimate") {
            // ETERNAL NEMESIS - 최종 보스 특별 렌더링
            ...
            const centerY = e.y + e.h/2;
            ctx.beginPath();
            ctx.moveTo(centerX, e.y); // 위
            ctx.lineTo(e.x + e.w, centerY); // 오른쪽
            ctx.lineTo(centerX, e.y + e.h); // 아래
            ctx.lineTo(e.x, centerY); // 왼쪽
            ctx.closePath();
            ctx.fill();
            
            // 중앙 코어
            ctx.fillStyle = isPhase3 ? "#ffffff" : (isPhase2 ? "#ff00ff" : "#ff0000");
    ...
    function update(dt) {
      ...
      if ((keysRef.current[" "] || keysRef.current["Space"]) && st.player.cooldown <= 0) {
        ...
        // 번개 공격 (레벨당 20% 확률, 더 많은 번개)
        const lightningLevel = playerStatsRef.current.lightningLevel || 0;
        if (playerStatsRef.current.hasLightning && skillChance < 0.20 * lightningLevel) {
          const count = 4 + Math.floor(lightningLevel * 1.5);
          for (let i = 0; i < count; i++) {
            const x = p.x + (Math.random() - 0.5) * 80;
            st.bullets.push({
        ...
        // 시간 왜곡 (레벨당 18% 확률, 더 많은 탄환)
        const timeWarpLevel = playerStatsRef.current.timeWarpLevel || 0;
        if (playerStatsRef.current.hasTimeWarp && skillChance < 0.18 * timeWarpLevel) {
          const bulletCount = 10 + timeWarpLevel * 3;
        ...
        // 별똥별 공격 (레벨당 15% 확률, 더 강력한 탄환)
        const starfallLevel = playerStatsRef.current.starfallLevel || 0;
        if (playerStatsRef.current.hasStarfall && skillChance < 0.15 * starfallLevel) {
          const count = 10 + starfallLevel * 2;
          for (let i = 0; i < count; i++) {
            st.bullets.push({
              x: p.x + (i - count/2) * 15,
              y: p.y - 10,
        ...
        // 카오스 공격 (레벨당 12% 확률, 더 많은 탄환과 강한 데미지)
        const chaosLevel = playerStatsRef.current.chaosLevel || 0;
        if (playerStatsRef.current.hasChaos && skillChance < 0.12 * chaosLevel) {
          const bulletCount = 12 + chaosLevel * 3;
          for (let i = 0; i < bulletCount; i++) {
            const angle = (i * 360 / bulletCount + Math.random() * 30) * Math.PI / 180;
            st.bullets.push({
              x: p.x + p.w / 2,
              y: p.y,
              w: 5,
  ...
  reverseTriggered = true;
          ...
          if (b.x < e.x + e.w && b.x + b.w > e.x && b.y < e.y + e.h && b.y + e.h > e.y) {
            ...
            if (e.boss && e.skill === "ultimate" && levelRef.current === 200) {
              ...
              if (e.phase2 && e.hp <= e.maxPhase2Hp * 0.3 && !e.phase3) {
                e.phase3 = true;
                e.hp = e.baseHp * 4; // 3페이즈만 체력 4배로 회복 (1페이즈 대비 2배 더 강화)
                e.maxPhase3Hp = e.baseHp * 4;
              ...
              if (e.hp <= 10 && !e.phase2) {
                e.phase2 = true;
                e.hp = e.baseHp * 2; // 체력 2배 회복
                e.maxPhase2Hp = e.baseHp * 2; // 2페이즈 최대 체력 저장
            ...
            if (e.hp <= 0) {
              ...
              if (wasBoss) {
                ...
                // teleport 보스(Shadow Warden, 30레벨)는 100% 확률로 텔레포트 제공
                if (bossSkill === "teleport" && bossSkillToUpgrade[bossSkill]) {
                  picks[0] = bossSkillToUpgrade[bossSkill];
                }
                // 다른 보스는 25% 확률로 보스 스킬을 업그레이드 목록에 추가
                else if (Math.random() < 0.25 && bossSkill && bossSkillToUpgrade[bossSkill]) {
      ...
      if (st.enemies.length === 0 && !st.nextWaveScheduled) {
        ...
        setTimeout(() => {
          ...
          // 200레벨 클리어 시 모든 비행기에 대해 게임 클리어 (무한 모드 제외)
          if (currentLevel >= 200 && !isInfiniteMode) {
            // 200레벨 완전 클리어!
            setGameOver(true); gameOverRef.current = true;
            setRunning(false); runningRef.current = false;
            setGameCleared(true);
